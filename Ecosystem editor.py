...
#METODO AUSILIARIO PER ELIMINARE UN OGGETTO DA UN GRUPPO
def removeobject(obj, gruppo):
			...
			for i in scena.keys():
				try:
					if(bpy.data.scenes[i].scene_property == "ECOSYS"):
						...
						oggetto.select = True
						bpy.context.scene.objects.active = oggetto
						bpy.ops.group.objects_remove(group = groupname)
...
bpy.utils.register_class(PathProperty)
...
class ParticleSystemList(bpy.types.PropertyGroup):
    pass
bpy.utils.register_class(ParticleSystemList)
...
class EcosystemLibrary(bpy.types.Panel):
	...
	def draw(self, context):
		...
		#LISTA DEI GRUPPI 
		row = layout.row()
		row.template_list("GroupEditor_ItemList", "", bpy.context.scene, "hk_group_list", bpy.context.scene, "hk_group_list_index")
...
class Set_Count(bpy.types.Operator):
	...
	def execute(self, context):
		...
		if( len(list_object) == 0 or list_object == None):
			return {'FINISHED'}
		#index_object = scena.hk_group_object_list_index
		#bpy.context.scene.hk_group_object_list_index = 0
		#bpy.context.scene.hk_group_object_list_index = GetSetFromName(self.target)
		print("target operator: "+str(self.target))
		index_object = GetSetFromName(self.target)
		scena.hk_group_object_list_index = index_object
		
		if(oggetto == None):
			#bpy.ops.error.message('INVOKE_DEFAULT', message = 'Select an ecosystem(object) in the scene!')
			return {'FINISHED'}
		...
		if( object_list_group_count == None):
			#bpy.ops.error.message('INVOKE_DEFAULT', message = 'Object not found in particle system\'s group!')
			return {'FINISHED'}
...
class SetNumberParticles(bpy.types.Operator):
	...
	def execute(self, context):
		oggetto = bpy.context.object
		list_ps = oggetto.particle_systems
		index_ps = GetSetFromNamePS(self.target)
		part_system = list_ps[index_ps]
		part_settings = bpy.data.particles[part_system.settings.name]
